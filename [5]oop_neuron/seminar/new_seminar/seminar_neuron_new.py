...
class Neuron:
    ...
    def backward_pass(self, X, y, y_pred, learning_rate=0.1):
        ...
        print(self.w.shape, (X.T @ ((y_pred - y) * y_pred * (1 - y_pred))).shape)
        self.w = self.w - learning_rate * (X.T @ ((y_pred - y) * y_pred * (1 - y_pred))) / n
        self.b = self.b - learning_rate * np.mean((y_pred - y) * y_pred * (1 - y_pred))
...
w[0][0] - (0.1 * (X.T @ ((y_pred - y) * y_pred * (1 - y_pred))) / 3)[0][0]


# In[82]:


(X.T @ ((y_pred - y) * y_pred * (1 - y_pred)))
...
(0.1 * (X.T @ ((y_pred - y) * y_pred * (1 - y_pred))) / 3)[0][0]
...
get_ipython().run_cell_magic('time', '', "\nneuron = Neuron()\nloss_values = neuron.fit(X, y, num_epochs=50000)  # num_epochs=50000\n\nplt.figure(figsize=(10, 8))\nplt.scatter(data.iloc[:, 0], data.iloc[:, 1], c=np.array(neuron.forward_pass(X) > 0.5).ravel(), cmap='spring')\nplt.title('Яблоки и груши', fontsize=15)\nplt.xlabel('симметричность', fontsize=14)\nplt.ylabel('желтизна', fontsize=14)\nplt.show();")
